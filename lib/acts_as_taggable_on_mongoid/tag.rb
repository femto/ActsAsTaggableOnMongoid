...
module ActsAsTaggableOnMongoid
  class Tag
    ...
    def self.named_like(name)
      clause = ["name #{ActsAsTaggableOnMongoid::Utils.like_operator} ? ESCAPE '!'", "%#{ActsAsTaggableOnMongoid::Utils.escape_like(name)}%"]
      where(clause)
    end

    def self.named_like_any(list)
      clause = list.map { |tag|
        sanitize_sql(["name #{ActsAsTaggableOnMongoid::Utils.like_operator} ? ESCAPE '!'", "%#{ActsAsTaggableOnMongoid::Utils.escape_like(tag.to_s)}%"])
      }.join(' OR ')
      where(clause)
    end
    ...
    class << self
      ...
      def binary
        ActsAsTaggableOnMongoid::Utils.using_mysql? ? 'BINARY ' : nil
      end
